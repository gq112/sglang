...
DeepSeek-V4 also ships DSpark block speculative decoding in the `*-DSpark` checkpoints (`deepseek-ai/DeepSeek-V4-Flash-DSpark`, `deepseek-ai/DeepSeek-V4-Pro-DSpark`). Enable it with `--speculative-algorithm DSPARK`; lowercase `dspark` is accepted and normalized, matching the intent of vLLM-style `method=dspark` configs. The drafter lives inside the checkpoint, so there is no separate draft model. It is lossless at `temperature 0` (and verifies `temperature > 0` with target-only speculative sampling) and gives roughly 1.5x to 1.8x single-stream speedup. On SM89/Ada systems such as RTX 4090, use `--moe-runner-backend marlin --speculative-moe-runner-backend marlin`. See [DSpark Decoding](../../../docs/advanced_features/speculative_decoding#dspark-decoding-deepseek-v4).
