...
For SM89/Ada systems such as 4x RTX 4090, use the Marlin MoE fallback instead of FlashInfer MXFP4:

```bash Command
python3 -m sglang.launch_server \
    --model-path deepseek-ai/DeepSeek-V4-Flash-DSpark \
    --trust-remote-code --tp 4 --ep-size 4 \
...
The SGLang CLI equivalent of a vLLM-style `method=dspark` setting is `--speculative-algorithm DSPARK`; lowercase `dspark` is accepted and normalized.
...
- On SM89/Ada, DSpark uses the Triton FlashMLA fallback and Marlin for target and speculative MoE experts; `flashinfer_mxfp4` and `deep_gemm` are not supported on this path.
